...
/** แปลง scan date: YYYY-MM-DD → DD/MM/YYYY */
...
function emptyFilters(): VolumeFilters {
  return { dateType: 'scan', dateFrom: '', dateTo: '', shift: '', model: '', line: '' }
}
...
function FilterPanel({ open, onClose, filters, setFilters, modelOpts, lineOpts, onApply, onReset }: {
  open: boolean; onClose: () => void
  filters: VolumeFilters; setFilters: (f: VolumeFilters) => void
  modelOpts: string[]; lineOpts: string[]
  onApply: () => void; onReset: () => void
}) {
  if (!open) return null
  const set = (patch: Partial<VolumeFilters>) => setFilters({ ...filters, ...patch })
  return (
    <>
          ...
          <div style={fpStyle.group}>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              <div style={{ display:'flex', alignItems:'center', gap:16 }}>
                <span style={fpStyle.label}>Date<br/><span style={{ fontSize:11, fontWeight:400, color:'#9ca3af' }}>(Production)</span></span>
              </div>
...
export default function VolumeTablePage() {
  ...
  const [filters,   setFilters]   = useState<VolumeFilters>(emptyFilters())
  const [applied,   setApplied]   = useState<VolumeFilters>(emptyFilters())
  ...
  const filtered = rows.filter(r => {
    ...
    const f = applied
    if (f.dateFrom && String(r.date_day ?? '') < f.dateFrom) return false
    if (f.dateTo   && String(r.date_day ?? '') > f.dateTo)   return false
    ...
    <div className="dfp-page" style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden', background:'#F8FAFC' }}>
      ...
      <div className="dfp-header-bar" style={{ background: 'transparent', borderBottom: 'none', paddingBottom: 0 }}>
        <div className="dfp-header-left">
          <h1 className="dfp-title">Volume Table</h1>
          <span className="dfp-breadcrumb">Data Table &gt; Volume Table</span>
        </div>
      </div>
      ...
      <div className="dfp-body">
        ...
        <FilterPanel
          ...
          modelOpts={modelOpts} lineOpts={lineOpts}
